...
def filter_products(products, max_price):
    ...
    limit = [item for item in products if item["price"] <= max_price]
    return limit
...
if __name__ == "__main__":
    ...
    if len(result) == 4 and {"name": "Laptop", "price": 999.99} not in result:
        print(
            "✅ Test 2 PASSED: filter_products(products, 49.99) excludes expensive items"
        )
    else:
        print(
            f"❌ Test 2 FAILED: Expected 4 items without Laptop, got {len(result) if result else 0} items"
        )
    ...
    if result == []:
        print(
            "✅ Test 3 PASSED: filter_products(products, 0.50) returns [] when nothing matches"
        )
    else:
